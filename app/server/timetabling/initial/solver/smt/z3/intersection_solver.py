import z3
from z3 import SolverFor, Int, Or, And, Implies, If, Real, ToInt, Not
...
class Z3IntersectionSolver(InitialBaseSolver):
    ...
    def init_constraints(self):
            ...
            for j, course_j in enumerate(self.courses):
                if i < j and course_i.group == course_j.group:
                    self.solver.add(Implies(
                        Not(Or(
                            self.tau_week_end[i] < self.tau_week_start[j],
                            self.tau_week_end[j] < self.tau_week_start[i],
                            self.tau_week_shift[i] != self.tau_week_shift[j],
                            self.tau_day[i] != self.tau_day[j],
                            self.tau_class_end[i] < self.tau_class_start[j],
                            self.tau_class_end[j] < self.tau_class_start[i],
                        )),
                        course_i.partition + course_j.partition <= 1
                    ))
        ...
        for i, course_i in enumerate(self.courses):
            for j, course_j in enumerate(self.courses):
                if i < j and course_i.lecturer == course_j.lecturer and \
                        (course_i.combined_code is None or course_i.combined_code != course_j.combined_code):
                    self.solver.add(Or(
                        self.tau_week_end[i] < self.tau_week_start[j],
                        self.tau_week_end[j] < self.tau_week_start[i],
                        self.tau_week_shift[i] != self.tau_week_shift[j],
